...
from graph_construction import (
    ...
    save_pyg_run_level_dataset 
)
from model_definition import get_model
from training import train_model, plot_training_history
from baseline import run_svm_baseline 
from explainability import analyze_attention, run_gnn_explainer 
...
def main():
    ...
    try:
        ...
        # Save the graph list and info using the correct function and ensure it uses num_classes and class_names
        dataset_storage_path = OUTPUT_DIR / 'pyg_run_level_dataset'
        saved_list_and_info_check = save_pyg_run_level_dataset(graph_list_created, num_classes, class_names, dataset_storage_path)
        if not saved_list_and_info_check: raise ValueError("Saving run-level graph list and/or info failed.")
    ...
    print("\n--- Stage 3: GAT Model Training ---")
    try:
        # Instantiate model using the ACTUAL number of classes (subjects)
        model_instance = get_model(num_classes=actual_num_classes)
    ...
    print("\n--- Stage 8: Robustness Check (Optional) ---")
    try:
        run_robustness_check(graph_list, dataset_info)
